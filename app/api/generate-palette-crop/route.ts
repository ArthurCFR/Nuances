...
export async function POST(request: NextRequest) {
  try {
    ...
    const scriptPath = path.join(process.cwd(), 'generate_palette_crop.py');
    ...
    const result = await new Promise<string>((resolve, reject) => {
      ...
      python.stderr.on('data', (data) => {
        ...
        console.log('[Python Palette Crop]', data.toString());
      });
    ...
    console.error('Palette crop generation error:', error);
    return NextResponse.json(
